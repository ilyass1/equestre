...
try {
 ...
 $connect = connection();
      $request = $connect->prepare('SELECT nom,prenom,cotisation,janvier,février,mars,avril,mai,juin,juillet,août,septembre,octobre,novembre,decembre FROM adherents a , adherent_payment ad WHERE a.adherent_code = ad.adherent_code'); 
      $request->execute();
}
    catch (Exception $e) {
      $e->getMessage();
  ...
  if($_SERVER['REQUEST_METHOD'] == "GET" && !empty($_GET['search'])  && isset($_GET['search']) ){
   $request=$connect->prepare('SELECT nom,prenom,cotisation,janvier,février,mars,avril,mai,juin,juillet,août,septembre,octobre,novembre,decembre FROM adherents a , adherent_payment ad WHERE a.adherent_code = ad.adherent_code and a.nom = ? and a.prenom = ? or a.nom =? and a.prenom =?'); 
    ...
    <tbody>
        <?php
         while($data=$request->fetch() ){
             ...
             ?>
             <tr>
        <td> <a href="payment.php?nom=<?php echo $data['nom'];?>&prenom=<?php echo $data['prenom'];?>&type=adherent"><?php echo $data['nom'].' '.$data['prenom'];?> </a></td>
        <td><?php echo $data['cotisation'];?></td>
