...
try {
 ...
 $connect = connection();
      $request = $connect->prepare('SELECT * FROM adherents'); 
      $request->execute();
}
    catch (Exception $e) {
      $e->getMessage();
  ...
  if($_SERVER['REQUEST_METHOD'] == "GET" && !empty($_GET['search'])  && isset($_GET['search']) ){
   $request=$connect->prepare('SELECT * FROM adherents where (nom = ? and prenom = ?) or (nom =? and prenom =?) or nom =? or prenom =?' ); 
...
<nav class="navbar text-center">
  <div class="container-fluid" >
    ...
    <div class="container text-center">
        ...
        <h1 id="title" >Liste des adhérents</h1> 
    ...
    <tbody>
        <?php
         while($data=$request->fetch() ){
             ?>
             <tr>
        <td><?php echo $data['adherent_code'];?></td>
